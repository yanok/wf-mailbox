...
struct hwfq * hwfq_alloc(int size, int element_size, int max_threads)
{
	...
	q->head = q->tail = q->newhead = q->dropped = 0;
...
static inline void unlock_head(struct hwfq *q, uint64_t h)
{
	...
	debug("Unlocking head, h = %llx, newhead = %llx, q->head = %llx\n",
		  h, q->newhead, q->head);
	if (!__sync_bool_compare_and_swap(&q->head, (h & ~q->lock_mask) | 1, q->newhead)) {
		debug("Not updating head, just unlocking, expected head = %llx\n",
			  (h & ~q->lock_mask) | 1);
